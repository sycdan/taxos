...
we need a flow test for renming a vendor (can be aprt of existing vendor flow)

are there any places we are still querying vendors by name? if so, we should siwtch to ID (guid)
