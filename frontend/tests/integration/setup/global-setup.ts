...
beforeAll(async () => {
  ...
  while (retries < maxRetries) {
    try {
      ...
      console.log('✅ Backend is ready, received:', JSON.stringify(result).slice(0, 100))
      break
    } catch (error) {
      console.log(`❌ Connection attempt ${retries + 1} failed:`, error instanceof Error ? error.message : String(error))
      retries++
      if (retries === maxRetries) {
        console.error('❌ Backend server not available after 5 retries')
        throw new Error(`Backend server not available after ${maxRetries} retries: ${error}`)
      }
