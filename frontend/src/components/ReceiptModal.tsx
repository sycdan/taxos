...
import { X, DollarSign, Calendar as CalendarIcon, Tag, Plus, Trash2, Lock, Unlock } from 'lucide-react';
...
const ReceiptModal: React.FC<ReceiptModalProps> = ({ 
  ...
  const [manualAllocations, setManualAllocations] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    if (isOpen) {
      if (editingReceipt) {
        ...
        setManualAllocations(new Set(editingReceipt.allocations.map(a => a.bucketId)));
      } else {
        setVendor('');
        setTotal(0);
  ...
  const updateSplits = (currentAllocations: ReceiptAllocation[], manualIds: Set<string>, newTotal: number) => {
    ...
    const splitCount = currentAllocations.filter(a => !manualIds.has(a.bucketId)).length;
    ...
    if (splitCount > 0 && remainingToSplit > 0) {
      ...
      nextAllocations = nextAllocations.map((a) => {
        ...
        const amount = isLast ? Number((remainingToSplit - distributed).toFixed(2)) : splitAmount;
        distributed += amount;
        return { ...a, amount };
      });
  ...
  const handleDelete = () => {
    if (editingReceipt && window.confirm('Are you sure you want to delete this receipt?')) {
  ...
  const availableBuckets = buckets.filter(b => !allocations.some(a => a.bucketId === b.id));
  ...
  return (
    <div className="modal-overlay" style={{
      ...
      <motion.div 
        ...
        className="card" 
        style={{ width: '100%', maxWidth: '640px', maxHeight: '95vh', overflowY: 'auto', padding: '2rem' }}
      >
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-black tracking-tight">{editingReceipt ? 'Edit Receipt' : 'New Receipt'}</h2>
            <p className="text-sm text-muted">{editingReceipt ? 'Update transaction details.' : 'Record details and allocate funds.'}</p>
          </div>
          <button className="btn btn-ghost p-2 rounded-full" onClick={onClose}><X size={20} /></button>
        </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <Tag size={16} className="text-muted" style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)' }} />
                  <input 
            ...
            <div className="space-y-6">
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <DollarSign size={20} className="text-primary" style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)' }} />
                  <input 
                    type="number" 
                    step="0.01"
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <CalendarIcon size={16} className="text-muted" style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)' }} />
                  <input 
                    type="datetime-local" 
            ...
            <div className="min-h-[60px]">
              <AnimatePresence initial={false}>
                {allocations.map((allocation, index) => {
                  const bucket = buckets.find(b => b.id === allocation.bucketId);
                  const isManual = manualAllocations.has(allocation.bucketId);
                  const percent = total > 0 ? (allocation.amount / total) * 100 : 0;
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <button 
                          ...
                          title={isManual ? "Manual entry - Click to auto-split" : "Auto-splitting - Click to lock amount"}
                        >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input 
                            ...
                            onChange={e => handleManualAmount(allocation.bucketId, parseFloat(e.target.value) || 0)}
                          />
                        </div>
                        
                        <span className="text-[10px] text-muted w-8 text-right font-mono font-bold">{percent.toFixed(0)}%</span>

                        <button 
                          onClick={() => handleRemoveBucket(allocation.bucketId)}
        ...
        <div className="mt-10 flex gap-4">
          {editingReceipt ? (
            <button className="btn btn-ghost flex-1 py-3 text-error hover:bg-error/10" onClick={handleDelete}>Delete Receipt</button>
          ) : (
            <button className="btn btn-ghost flex-1 py-3" onClick={onClose}>Cancel</button>
          )}
          <button 
