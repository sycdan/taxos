    ...
    >
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted">Create and organize your spending categories.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleAdd} className="mb-12 flex flex-col gap-2">
        <div className="flex gap-4">
          ...
          />
          <button type="submit" className="btn btn-primary" disabled={!newBucketName.trim() || isNameTaken(newBucketName)}>
            <Plus size={20} />
            <span>Add Bucket</span>
          </button>
        </div>
        {newBucketName.trim() && isNameTaken(newBucketName) && (
          <div className="text-error text-xs font-semibold ml-2">This bucket name is already taken.</div>
        )}
      </form>
                ...
                {editingId === bucket.id ? (
                  <>
                    <button 
                      ...
                      onClick={saveEdit}
                      disabled={!editName.trim() || isNameTaken(editName, bucket.id)}
                    >
                      <Check size={18} />
                    </button>
                    <button className="btn btn-ghost p-1 text-muted" onClick={() => setEditingId(null)}>
                      <X size={18} />
                    </button>
                  </>
                ) : (
                  <>
                    <button className="btn btn-ghost p-1 text-muted group-hover:text-primary transition-colors" onClick={() => startEditing(bucket)}>
                      <Edit2 size={18} />
                    </button>
                    <button 
            ...
            {editingId === bucket.id ? (
              <div className="flex flex-col gap-1">
                ...
                {editName.trim() && isNameTaken(editName, bucket.id) && (
                  <div className="text-error text-[10px] font-semibold">Name already taken</div>
                )}
              </div>
            ) : (
              <div 
